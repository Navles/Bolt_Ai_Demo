...
const COLUMN_MAPPINGS = {
  ...
  'productCode': ['product code', 'code', 'item code', 'material code'],
};

export default function ExcelUpload({ open, onClose, onImport, costHead }: ExcelUploadProps) {
  ...
  const handleImport = () => {
    ...
    const importItems = validRows.map(row => ({
      ...
      productCode: row.productCode || `EXCEL-${Date.now()}`,
  ...
  const downloadTemplate = () => {
    const templateData = [
      ['Description', 'Quantity', 'Unit', 'Unit Cost', 'Product Code'],
      ['Cement - OPC 53 Grade', 100, 'Bags', 450, 'MAT-CEM-001'],
      ['Steel Bars - 12mm TMT', 5, 'Tons', 65000, 'MAT-STL-001'],
      ['Skilled Mason', 10, 'Days', 800, 'LAB-SKL-001'],
    ];

    const ws = XLSX.utils.aoa_to_sheet(templateData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Cost Estimation Template');
    XLSX.writeFile(wb, 'cost_estimation_template.xlsx');
  };
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="xl" fullWidth>
      <DialogTitle>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6">Import Cost Estimation from Excel</Typography>
          <Button
            startIcon={<Download />}
      ...
      <DialogContent>
        {!costHead && (
          <Alert severity="warning" sx={{ mb: 3 }}>
            Please select a Cost Head in the main form before importing Excel data.
          </Alert>
        )}
        ...
        {uploadedData.length === 0 ? (
          <Box>
            <Alert severity="info" sx={{ mb: 3 }}>
              Upload an Excel file with columns: Description, Quantity, Unit, Unit Cost, and optionally Product Code.
                      ...
                      <TableCell>
                        {editingRow === row.id ? (
                          <TextField
                            ...
                            onChange={(e) => handleEditRow(row.id, 'productCode', e.target.value)}
                            onBlur={() => setEditingRow(null)}
                            autoFocus
                          />
                        ) : (
                          <Box onClick={() => setEditingRow(row.id)} sx={{ cursor: 'pointer' }}>
                            {row.productCode || 'Auto-generated'}
                          </Box>
                        )}
                      </TableCell>
                      <TableCell>
      ...
      <DialogActions>
        ...
        {uploadedData.length > 0 && (
          <Button
            ...
            disabled={validRowsCount === 0 || !costHead}
